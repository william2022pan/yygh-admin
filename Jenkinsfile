pipeline {
    ...
    stages {
        stage('拉取代码') {
            ...
            steps {
                container('nodejs') {
                    git(url: 'https://github.com/william2022pan/yygh-admin.git', credentialsId: 'github-id', branch: 'master', changelog: true, poll: false)
                    sh 'ls -al'
                }
        ...
        stage('发送确认邮件') {
            agent none
            steps {
                mail(to: 'anger_god@163.com', subject: 'yygh-admin构建结果', body: "构建成功了  $BUILD_NUMBER")
            }
        }
